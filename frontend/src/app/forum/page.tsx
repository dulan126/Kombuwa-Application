...
export default function ForumPage() {
  ...
  useEffect(() => {
    ...
    loadThreads(filter);
  }, [isLoggedIn, router, filter]);

  const loadThreads = useCallback(async (f: string) => {
    setLoading(true);
    // Demo data
    const filtered = DEMO_THREADS.filter((t) => {
      if (f === FORUM_FILTERS.ALL) return true;
      if (f === FORUM_FILTERS.RESOLVED || f === FORUM_FILTERS.PENDING) return t.status === f;
      return t.subject_id === f;
    });
    setThreads(filtered);
    setLoading(false);
  }, []);
  ...
  const openThread = (id: string) => {
    const demo = DEMO_THREADS.find((t) => t.id === id);
    if (!demo) return;
    setThreadDetail({
      thread: demo,
      replies: demo.answers.map((a, i) => ({
        id: String(i),
        body: a.t,
        is_verified: a.v,
        name: a.n,
  ...
  if (selectedThread && threadDetail) {
    ...
    return (
      <div className="p-6 min-h-[calc(100vh-58px)]">
        <button className="flex items-center gap-1.5 text-xs text-text-muted bg-transparent border-none cursor-pointer mb-4 font-[inherit] hover:text-text-primary" onClick={() => setSelectedThread(null)}>
          ← Forum
        </button>
          ...
          >
            ...
            <div className="flex items-center gap-2.5 mb-2">
              ...
              <div>
                ...
                <div className="text-[10px] text-text-muted">{r.role}</div>
              </div>
            </div>
        ...
        <div className="max-w-[760px] mt-5">
          ...
          />
          <Button className="mt-2.5">Post Reply</Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-[220px_1fr] max-md:grid-cols-1 min-h-[calc(100vh-58px)]">
      ...
      <div className="bg-dark-2 border-r border-border-dim p-5 max-md:hidden">
        ...
        {SUBJECT_FILTERS.map((sf) => (
          <button
            ...
            className={cn(
              ...
              'hover:bg-surface',
              filter === sf.id && 'bg-gold-bg text-gold',
              filter !== sf.id && 'text-text-primary',
            )}
        ...
        <button className={cn('flex items-center gap-2 px-2.5 py-[7px] rounded-[var(--radius-sm)] cursor-pointer text-[11.5px] w-full bg-transparent border-none text-left font-[inherit]', filter === FORUM_FILTERS.RESOLVED ? 'bg-gold-bg text-gold' : 'text-text-primary hover:bg-surface')} onClick={() => setFilter(FORUM_FILTERS.RESOLVED)}>
          <span className="w-[7px] h-[7px] rounded-full bg-success shrink-0" />විසඳිණ
        </button>
        <button className={cn('flex items-center gap-2 px-2.5 py-[7px] rounded-[var(--radius-sm)] cursor-pointer text-[11.5px] w-full bg-transparent border-none text-left font-[inherit]', filter === FORUM_FILTERS.PENDING ? 'bg-gold-bg text-gold' : 'text-text-primary hover:bg-surface')} onClick={() => setFilter(FORUM_FILTERS.PENDING)}>
      ...
      <div className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-[11px] text-text-muted mt-0.5">1,248 ප්‍රශ්න — මෙම මාසය</p>
          </div>
