...
CREATE TABLE IF NOT EXISTS users (
  ...
  stream        stream_enum,                          -- NULL for teachers/admins
  grade         grade_enum,
...
CREATE TABLE IF NOT EXISTS subjects (
  id          VARCHAR(10)   PRIMARY KEY,   -- 'm','ph','ch','bi','ac','ec'...
...
CREATE TABLE IF NOT EXISTS papers (
  ...
  question_count  SMALLINT      NOT NULL,        -- 10 for daily, 30 for SRP
  time_seconds    INT           NOT NULL,         -- 600 or 1800
  available_from  TIMESTAMPTZ   NOT NULL,         -- when students can access
  available_until TIMESTAMPTZ,                    -- SRP window close; NULL = open-ended
  ms_available    BOOLEAN       NOT NULL DEFAULT FALSE,
  ms_available_at TIMESTAMPTZ,                    -- when marking scheme was uploaded
...
CREATE TABLE IF NOT EXISTS questions (
  ...
  explanation     TEXT,                           -- shown in marking scheme
  image_url       TEXT                            -- optional image in question
);
...
CREATE TABLE IF NOT EXISTS attempts (
  ...
  answers         JSONB         NOT NULL DEFAULT '{}',  -- {"0":"A","1":"C",...}
...
-- ═══════════════════════════════════════════════════════════════════
-- RANKINGS (materialised per paper)
-- ═══════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS past_papers (
  ...
  essay_pdf_url     TEXT,                           -- S3 / local path
  essay_pdf_size    INT,
...
CREATE TABLE IF NOT EXISTS pp_questions (
  ...
  option_c        TEXT          NOT NULL,
  option_d        TEXT          NOT NULL,
  correct_option  CHAR(1)       CHECK (correct_option IN ('A','B','C','D')),  -- NULL until MS uploaded
  image_url       TEXT
);
...
CREATE TABLE IF NOT EXISTS forum_threads (
  ...
  body          TEXT            NOT NULL,
  image_urls    TEXT[]          NOT NULL DEFAULT '{}',  -- up to 3 images
...
CREATE TABLE IF NOT EXISTS forum_replies (
  ...
  user_id       UUID            NOT NULL REFERENCES users(id),
  body          TEXT            NOT NULL,
  is_verified   BOOLEAN         NOT NULL DEFAULT FALSE,  -- teacher-verified answer
